...
//    }
    ...
    public static int getAppVersionCode(ApplicationContext mContext) {
        //包管理操作管理类
...
//        }
    ...
    public static String getAppVersionName(ApplicationContext mContext) {
        //包管理操作管理类
//        PackageManager pm = mContext.getPackageManager();
//        try {
//            PackageInfo packageInfo = pm.getPackageInfo(mContext.getPackageName(), 0);
...
//        }
    ...
    public static String checkReplaceProxy(String urlOri) {
        if (urlOri.startsWith("proxy://")) {
            return urlOri.replace("proxy://", RemoteServerUtil.getAddress(true) + "proxy?");
        }
        return urlOri;
    }
}